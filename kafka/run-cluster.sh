...
# Launch container 2
docker run -d --name "${KAFKA_NAME}" -p 29092:29092 --network "${NETWORK_NAME}" \
    -e KAFKA_LISTENERS="EXTERNAL_SAME_HOST://:29092,INTERNAL://:9092" -e KAFKA_ADVERTISED_LISTENERS="INTERNAL://kafka-server:9092,EXTERNAL_SAME_HOST://localhost:29092" \
    -e KAFKA_LISTENER_SECURITY_PROTOCOL_MAP="INTERNAL:PLAINTEXT,EXTERNAL_SAME_HOST:PLAINTEXT" \
    -e KAFKA_INTER_BROKER_LISTENER_NAME="INTERNAL" \
    -e ALLOW_PLAINTEXT_LISTENER=yes -e KAFKA_CFG_ZOOKEEPER_CONNECT=zookeeper-server:2181 bitnami/kafka:latest
